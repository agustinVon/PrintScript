...
case class Matches() {
  def getMatches: Map[TokenType, LexerMatcher] = Map(
    ...
    TokenTypesImpl.STRING -> LexerMatcherImpl.fromRegex(TokenTypesImpl.TYPESTRING,"string"),
    TokenTypesImpl.NUMBER -> LexerMatcherImpl.fromRegex(TokenTypesImpl.TYPENUMBER, "number"),
